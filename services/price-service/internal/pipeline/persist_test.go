...
func TestPhase3SqlcImport(t *testing.T) {
	...
	var _ sqlcgen.GetStorePreviousPriceParams
...
// TestPhase3ComputePriceSignature verifies price signature computation
func TestPhase3ComputePriceSignature(t *testing.T) {
	discountPrice := 800
	row := types.NormalizedRow{
		Price:         1000,
	...
	sig := computePriceSignature(row)

	// Verify signature contains the price
	if !strings.Contains(sig, "1000") {
		t.Errorf("price signature should contain price 1000, got: %s", sig)
	}

	// Verify signature is deterministic
	sig2 := computePriceSignature(row)
	if sig != sig2 {
		t.Errorf("price signature should be deterministic")
	}

	// Verify different rows produce different signatures
	row2 := types.NormalizedRow{
		Price: 2000,
	}
	sig3 := computePriceSignature(row2)
	if sig == sig3 {
		t.Errorf("different rows should produce different signatures")
	}
}
...
// TestPhase3UpsertStoreItemStateNoID verifies ID is not required for store_item_state
func TestPhase3UpsertStoreItemStateNoID(t *testing.T) {
	paramsType := reflect.TypeOf(sqlcgen.UpsertStoreItemStateParams{})

	// ID should NOT be a field (it's auto-generated by sequence)
	_, found := paramsType.FieldByName("ID")
	if found {
		t.Error("UpsertStoreItemStateParams should not have ID field (auto-generated by sequence)")
	}

	// StoreID and RetailerItemID should exist
	requiredFields := []string{"StoreID", "RetailerItemID", "CurrentPrice", "PreviousPrice", "PriceSignature"}
	for _, fieldName := range requiredFields {
		_, found := paramsType.FieldByName(fieldName)
		if !found {
			t.Errorf("UpsertStoreItemStateParams missing field: %s", fieldName)
		}
	}
}
...
func TestPhase3PersistPhaseSignature(t *testing.T) {
	...
	if fnType.NumIn() != 6 {
		t.Errorf("PersistPhase should have 6 parameters, got %d", fnType.NumIn())
	}

...
const ERROR_TYPE_LABELS: Record<string, { label: string; description: string }> = {
  ...
  validation: { label: 'Validation Errors', description: 'Data validation failures' },
  store_resolution: { label: 'Store Resolution', description: 'Could not match store identifier' },
  persist: { label: 'Persist Errors', description: 'Failed to save data to database' },
...
export function ErrorCategoryView({ errors, isLoading }: ErrorCategoryViewProps) {
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
  ...
  const sortedByType = Object.entries(errors.byType).sort(([, a], [, b]) => b - a)
  const sortedBySeverity = Object.entries(errors.bySeverity).sort(([, a], [, b]) => b - a)
        ...
        <div>
          ...
          <div className="space-y-2">
            {sortedByType.map(([type, count]) => {
              const typeInfo = ERROR_TYPE_LABELS[type] || { label: type, description: 'Unknown error type' }
              const percentage = Math.round((count / errors.total) * 100)

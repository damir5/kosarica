import { Clock, CheckCircle, XCircle, Loader2, Database, Package } from 'lucide-react'
...
const STATUS_COLORS: Record<ChunkStatus, 'secondary' | 'default' | 'destructive'> = {
  pending: 'secondary',
  processing: 'default',
  completed: 'default',
  failed: 'destructive',
      ...
      <Table>
        ...
        <TableBody>
          {chunks.map((chunk) => {
            ...
            const successRate = chunk.rowCount > 0
            ...
            return (
              <TableRow key={chunk.id}>
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-mono">{chunk.id}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="text-sm">
                    <span className="font-medium">{chunk.rowCount.toLocaleString()}</span>
                    <span className="text-muted-foreground"> rows</span>
                    <p className="text-xs text-muted-foreground">
                      {chunk.startRow.toLocaleString()} - {chunk.endRow.toLocaleString()}
                    </p>
                  </div>
                </TableCell>
                <TableCell>
                    ...
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                      <div
                        className={`h-full transition-all duration-300 ${
                          status === 'failed' ? 'bg-destructive' : 'bg-green-500'
                        }`}
                ...
                <TableCell>
                  <span className={`font-medium ${errorCount > 0 ? 'text-destructive' : 'text-muted-foreground'}`}>
                ...
                <TableCell>
                  {onRerunChunk && (status === 'completed' || status === 'failed') && (
                    <RerunButton

...
export function IngestionRunCard({ run, onRerun, isRerunning }: IngestionRunCardProps) {
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline">{SOURCE_LABELS[run.source] || run.source}</Badge>
            <Badge
        ...
        <div className="space-y-3">
          ...
          {status === 'running' && totalFiles > 0 && (
            <div>
              <div className="flex justify-between text-xs text-muted-foreground mb-1">
                <span>Progress</span>
                <span>{processedFiles} / {totalFiles} files ({progress}%)</span>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium">{processedFiles} / {totalFiles}</p>
            </div>
            <div>
              <span className="text-muted-foreground">Entries</span>
              <p className="font-medium">
                {(run.processedEntries ?? 0).toLocaleString()} / {(run.totalEntries ?? 0).toLocaleString()}
              </p>
            </div>
            <div>
              <span className="text-muted-foreground">Errors</span>
              <p className={`font-medium ${(run.errorCount ?? 0) > 0 ? 'text-destructive' : ''}`}>
                {run.errorCount ?? 0}
              </p>
            </div>
            <div>

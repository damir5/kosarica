import { Activity, FileText, AlertTriangle, Database, Clock, CheckCircle } from 'lucide-react'
...
export function IngestionStatsCards({ stats, isLoading }: IngestionStatsCardsProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="font-bold text-2xl">{filesProcessed.toLocaleString()}</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className={`font-bold text-2xl ${totalErrors > 0 ? 'text-destructive' : ''}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="font-bold text-2xl">{rowsIngested.toLocaleString()}</div>
          <p className="text-xs text-muted-foreground">

import { MapPin, CheckCircle2, XCircle, Clock, AlertCircle, Loader2 } from 'lucide-react'
...
export function EnrichmentTaskCard({ task, onVerify }: EnrichmentTaskCardProps) {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {task.confidence && (
              <Badge variant={CONFIDENCE_COLORS[task.confidence as keyof typeof CONFIDENCE_COLORS] || 'outline'}>
      ...
      <CardContent>
        ...
        {/* Geocode Results */}
        {task.type === 'geocode' && task.status === 'completed' && outputData && (
            ...
            {outputData.found ? (
              <>
                ...
                {outputData.displayName && (
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">Matched Address</label>
                    <p className="text-sm text-muted-foreground">{outputData.displayName}</p>
                  </div>
                )}
                {/* Map link */}
                <a
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <p className="text-sm">No geocoding results found for this address</p>
              </div>
            )}
          </div>
        )}
        ...
        {task.type === 'verify_address' && task.status === 'completed' && outputData && (
          <div className="space-y-3">
            ...
            <div className="grid gap-2">
              <div>
                <label className="text-xs font-medium text-muted-foreground">Original Address</label>
                <p className="text-sm">{outputData.originalAddress || 'Not set'}</p>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">{outputData.postalCode || 'Not set'}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {task.type === 'ai_categorize' && task.status === 'completed' && outputData && (

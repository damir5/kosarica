...
import { Brain, FileCheck, Loader2, MapPin, Sparkles } from "lucide-react";
...
import { type EnrichmentTask, EnrichmentTaskCard } from "./EnrichmentTaskCard";
...
export function StoreEnrichmentSection({
	storeId,
	store,
}: StoreEnrichmentSectionProps) {
	...
	const hasActiveVerifyAddress = tasks.some(
		(t) =>
			t.type === "verify_address" &&
			(t.status === "pending" || t.status === "processing"),
	);
	const hasActiveAiCategorize = tasks.some(
		(t) =>
			t.type === "ai_categorize" &&
			(t.status === "pending" || t.status === "processing"),
	);
		...
		<>
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Automatically enhance store data with geocoding and AI-powered
						verification
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-6">
					...
					<div className="flex flex-wrap gap-2">
						...
						>
							...
							triggerMutation.variables === "geocode" ? (
						...
						>
							{triggerMutation.isPending &&
							triggerMutation.variables === "verify_address" ? (
								<Loader2 className="mr-2 h-4 w-4 animate-spin" />
							) : (
						...
						>
							{triggerMutation.isPending &&
							triggerMutation.variables === "ai_categorize" ? (
								<Loader2 className="mr-2 h-4 w-4 animate-spin" />
							) : (
					...
					<div className="space-y-4">
						...
						{tasks.length === 0 && !isLoadingTasks ? (
							<div className="rounded-md bg-muted p-4 text-center text-sm text-muted-foreground">
								No enrichment tasks yet. Click a button above to start enriching
								this store's data.
							</div>
						) : (
							<div className="space-y-4">

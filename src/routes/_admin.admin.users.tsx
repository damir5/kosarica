...
function UsersPage() {
  ...
  const [modalType, setModalType] = useState<'view' | 'edit' | 'delete' | 'ban' | null>(null)
  ...
  const { data, isLoading, error } = useQuery(
    orpc.admin.users.list.queryOptions({
      input: {
        ...
        role: roleFilter !== 'all' ? (roleFilter as 'user' | 'superadmin') : undefined,
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ userId, role }: { userId: string; role: 'user' | 'superadmin' }) => {
  ...
  const handleOpenModal = (user: User, type: 'view' | 'edit' | 'delete' | 'ban') => {

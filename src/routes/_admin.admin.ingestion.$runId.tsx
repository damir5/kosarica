...
function RunDetailPage() {
  ...
  const { data: filesData, isLoading: filesLoading } = useQuery(
    orpc.admin.ingestion.listFiles.queryOptions({
      input: {
        runId,
        status: statusFilter !== 'all' ? (statusFilter as FileStatus) : undefined,
        page,
        pageSize,
      },
    }),
  ...
  const StatusIcon = STATUS_ICONS[run.status as keyof typeof STATUS_ICONS] || Clock
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground text-sm font-mono">{run.id}</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge
                variant={STATUS_COLORS[run.status as keyof typeof STATUS_COLORS] || 'secondary'}
              ...
              {run.parentRunId && (
                <Badge variant="outline">Rerun</Badge>
              )}
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${(run.errorCount ?? 0) > 0 ? 'text-destructive' : ''}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {run.parentRunId && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Parent Run</label>
                  <p className="mt-1">
                    <a
              ...
              {run.rerunType && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Rerun Type</label>
            ...
            {filesData && filesData.totalPages > 1 && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  ...
                  {Math.min(page * pageSize, filesData.total)} of {filesData.total} files
                </p>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"

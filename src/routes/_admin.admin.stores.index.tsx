...
function StoresPage() {
	...
	const [unlinkModalStore, setUnlinkModalStore] =
		useState<PhysicalStore | null>(null);
	const [selectedPriceSource, setSelectedPriceSource] = useState<string>("");
	const [addModalOpen, setAddModalOpen] = useState(false);
	const [addModalChain, setAddModalChain] = useState<{
		slug: string;
		name: string;
	} | null>(null);
	...
	const { data: virtualData, isLoading: virtualLoading } = useQuery(
		orpc.admin.stores.listVirtual.queryOptions({
			input: {
				...
				status:
					statusFilter !== "all"
						? (statusFilter as "active" | "pending")
						: undefined,
			...
			input: {
				...
				status:
					statusFilter !== "all"
						...
						: undefined,
				linkedStatus:
					linkedStatusFilter !== "all"
						? (linkedStatusFilter as "linked" | "unlinked")
						: undefined,
				search: debouncedSearch || undefined,
			},
		}),
	...
	const linkMutation = useMutation({
		mutationFn: async ({
			...
			return orpc.admin.stores.linkPriceSource.call({
				storeId,
				priceSourceStoreId,
			});
		},
	...
	return (
		<>
			...
			<div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
				...
				<div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center">
					...
					<div className="flex flex-wrap gap-2">
						...
						<Select
							value={linkedStatusFilter}
							onValueChange={(value) => {
								setLinkedStatusFilter(value);
								setPhysicalPage(1);
							}}
						>
							<SelectTrigger className="w-[140px]">
					...
					<div className="space-y-8">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									These stores are created automatically by your ingestion
									pipeline. Physical locations inherit prices from these
									sources.
								</CardDescription>
							</CardHeader>
							<CardContent>
							...
							<CardHeader>
								<div className="flex items-center justify-between">
									<div>
										...
										<CardDescription>
											Real-world store addresses. They inherit prices from a
											virtual source or have their own.
										</CardDescription>
									</div>
									<Select
										value=""
									...
									<>
										...
										<div className="mt-4 flex items-center justify-between">
											<p className="text-sm text-muted-foreground">
												...
												{Math.min(physicalPage * 20, physicalData.total)} of{" "}
											...
											<div className="flex items-center gap-2">
												<Button
													...
													size="sm"
													onClick={() =>
														setPhysicalPage((p) => Math.max(1, p - 1))
													}
												...
												<Button
													...
													disabled={
														physicalPage >= (physicalData.totalPages || 1)
													}
												>
													Next
			...
			<Dialog
				open={!!linkModalStore}
				onOpenChange={(open) => !open && setLinkModalStore(null)}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Link Price Source</DialogTitle>
						<DialogDescription>
							Select a virtual price source to link to "{linkModalStore?.name}".
							This store will inherit prices from the selected source.
						</DialogDescription>
					</DialogHeader>
					<div className="py-4">
						<Select
							value={selectedPriceSource}
							onValueChange={setSelectedPriceSource}
						>
							<SelectTrigger>
			...
			<Dialog
				open={!!unlinkModalStore}
				onOpenChange={(open) => !open && setUnlinkModalStore(null)}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Unlink Price Source</DialogTitle>
						<DialogDescription>
							Are you sure you want to unlink "{unlinkModalStore?.name}" from
							its price source "{unlinkModalStore?.priceSourceName}"? This store
							will no longer inherit prices.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>

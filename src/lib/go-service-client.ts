...
const GO_SERVICE_BASE_URL = process.env.GO_SERVICE_BASE_URL || 'http://localhost:8081';
const INTERNAL_API_KEY = process.env.INTERNAL_API_KEY || 'dev-internal-api-key-change-in-production';

async function goFetch(path: string, options?: RequestInit): Promise<GoServiceResponse> {
...
export async function scheduleIngestion(chainId: string, targetDate?: string): Promise<{ id: string }> {

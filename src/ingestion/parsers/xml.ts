...
export class XmlParser extends Parser {
	...
	protected async parseRows(
		...
		if (itemsArray.length === 0) {
			context.addWarning({
				field: null,
				message: "XML file contains no items",
			});
			return { rows: [], totalRows: 0 };
		}
	...
	private mapItemToNormalized(
		...
		const discountPriceStr = this.extractStringValue(
			item,
		...
		const storeIdentifier =
			this.extractStringValue(item, mapping.storeIdentifier) ??
		...
		const lowestPrice30dStr = this.extractStringValue(
			item,
			mapping.lowestPrice30d,
		);
		...
		const row: NormalizedRow = {
			...
			// Croatian price transparency fields
			unitPrice,
			unitPriceBaseQuantity: this.extractStringValue(
				item,
				mapping.unitPriceBaseQuantity,
			),
			unitPriceBaseUnit: this.extractStringValue(
				item,
				mapping.unitPriceBaseUnit,
			),
			lowestPrice30d,
			anchorPrice,
			anchorPriceAsOf,

...
const DEFAULT_OPTIONS: Required<
	Omit<CsvParserOptions, "columnMapping" | "defaultStoreIdentifier">
> = {
	delimiter: ",",
	encoding: "utf-8",
	hasHeader: true,
	...
	protected async parseRows(
		...
		if (!columnIndices) {
			context.addError({
				field: null,
				message:
					"No column mapping provided. Cannot map CSV columns to normalized fields.",
				originalValue: null,
			});
		...
		for (let i = dataStartRow; i < rawRows.length; i++) {
			if (
				!opts.skipEmptyRows ||
				!rawRows[i].every((cell) => cell.trim() === "")
			) {
				totalRows++;
			}
		}
...
export function detectDelimiter(content: string): CsvDelimiter {
	...
	for (const delimiter of delimiters) {
		...
		const variance =
			counts.reduce((sum, c) => sum + (c - avgCount) ** 2, 0) / counts.length;

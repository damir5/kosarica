...
export class KauflandAdapter extends BaseCsvAdapter {
	...
	private static readonly ASSET_API_URL =
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		console.log(
			`[DEBUG] Fetching Kaufland asset API: ${KauflandAdapter.ASSET_API_URL}`,
		);
		...
		try {
			...
			if (!response.ok) {
				console.error(
					`Failed to fetch Kaufland asset API: ${response.status} ${response.statusText}`,
				);
				return [];
			}
			...
			if (discoveredFiles.length === 0) {
				console.log(
					`[DEBUG] No CSV files found for date ${date} (pattern: ${targetDatePattern})`,
				);
			} else {
				console.log(
					`[DEBUG] Found ${discoveredFiles.length} CSV file(s) for date ${date}`,
				);
			}
		} catch (error) {
			const errorMessage =

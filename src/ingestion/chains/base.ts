...
export abstract class BaseChainAdapter implements ChainAdapter {
	...
	constructor(adapterConfig: BaseAdapterConfig) {
		...
		this.fileExtensionPattern =
			adapterConfig.fileExtensionPattern ?? /\.(csv|CSV)$/;
		this.rateLimitConfig = {
			...DEFAULT_RATE_LIMIT_CONFIG,
			...adapterConfig.rateLimitConfig,
		};
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		try {
			...
			if (!response.ok) {
				console.error(
					`Failed to fetch ${this.name} portal: ${response.status} ${response.statusText}`,
				);
	...
	protected extractFilenameFromUrl(url: string): string {
		try {
			...
			const filename =
				pathname.split("/").pop() || `unknown.${this.supportedTypes[0]}`;
...
export abstract class BaseCsvAdapter extends BaseChainAdapter {
	...
	constructor(adapterConfig: CsvAdapterConfig) {
		...
		if (!this.config.csv) {
			throw new Error(
				`${this.name} adapter requires CSV configuration but none was provided`,
			);
		}
	...
	async parse(
		...
		let result = await this.csvParser.parse(
			processedContent,
			filename,
			options,
		);
...
export abstract class BaseXmlAdapter extends BaseChainAdapter {
	...
	protected override extractStoreIdentifierFromFilename(
		...
		const storeIdMatch = cleanName.match(

...
export class KtcAdapter extends BaseCsvAdapter {
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		try {
			...
			if (!mainResponse.ok) {
				console.error(
					`[ERROR] Failed to fetch KTC portal: ${mainResponse.status} ${mainResponse.statusText}`,
				);
				return [];
			}
			...
			for (const storeName of stores) {
				...
				try {
					...
					if (!storeResponse.ok) {
						console.warn(
							`[WARN] Failed to fetch store page for ${storeName}: ${storeResponse.status}`,
						);
						continue;
					}
					...
					console.warn(
						`[WARN] Error fetching store ${storeName}: ${errorMessage}`,
					);
				}
			}
	...
	protected override extractStoreIdentifierFromFilename(
		...
		if (simpleMatch) {
			...
			const afterPJ = decodedFilename.substring(
				decodedFilename.indexOf(simpleMatch[1]),
			);

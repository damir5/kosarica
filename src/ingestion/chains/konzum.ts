...
export class KonzumAdapter extends BaseCsvAdapter {
	...
	constructor() {
		super({
			...
			filenamePrefixPatterns: [/^Konzum[_-]?/i, /^cjenik[_-]?/i],
			rateLimitConfig: {
				requestsPerSecond: 2,
				maxRetries: 3,
			},
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		while (page <= maxPages) {
			...
			try {
				...
				if (!response.ok) {
					console.error(
						`Failed to fetch Konzum portal page ${page}: ${response.status} ${response.statusText}`,
					);
				...
				const downloadPattern =
					/href=["'](\/cjenici\/download\?title=([^"'&]+)[^"']*)["']/gi;
				...
				console.error(
					`Error discovering Konzum files on page ${page}: ${errorMessage}`,
				);
				console.error(`  URL: ${pageUrl}`);
				break;
			}

...
export class MetroAdapter extends BaseCsvAdapter {
	constructor() {
		super({
			...
			filenamePrefixPatterns: [/^Metro[_-]?/i, /^cjenik[_-]?/i],
			rateLimitConfig: {
				requestsPerSecond: 2,
				maxRetries: 3,
			},
	 ...
	 */
	override extractStoreIdentifier(
		file: import("../core/types").DiscoveredFile,
	): import("../core/types").StoreIdentifier | null {
	 ...
	 */
	protected override getParserOptions(
		storeIdentifier: string,
	): import("../parsers/csv").CsvParserOptions {
		...
		const storeCode =
			this.extractStoreCodeFromFilename(storeIdentifier) || storeIdentifier;
		return {

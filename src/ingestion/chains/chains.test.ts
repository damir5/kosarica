...
// Import all adapters
import { KonzumAdapter, createKonzumAdapter } from './konzum'
import { LidlAdapter, createLidlAdapter } from './lidl'
import { PlodineAdapter, createPlodineAdapter } from './plodine'
import { IntersparAdapter, createIntersparAdapter } from './interspar'
import { StudenacAdapter, createStudenacAdapter } from './studenac'
import { KauflandAdapter, createKauflandAdapter } from './kaufland'
import { EurospinAdapter, createEurospinAdapter } from './eurospin'
import { DmAdapter, createDmAdapter } from './dm'
import { KtcAdapter, createKtcAdapter } from './ktc'
...
function createDiscoveredFile(filename: string, type: 'csv' | 'xml' | 'xlsx' = 'csv'): DiscoveredFile {
  return {
...
function createEmptyCSV(delimiter: string = ','): ArrayBuffer {
  const content = ''
...
function createHeaderOnlyCSV(headers: string[], delimiter: string = ','): ArrayBuffer {
...
// Helper function to create CSV with invalid price data
function createCSVInvalidPrices(delimiter: string = ','): ArrayBuffer {
  const headers = `Naziv${delimiter}Cijena${delimiter}Barkod`
  const rows = [
    `Product1${delimiter}not_a_price${delimiter}1234567890123`,
    `Product2${delimiter}${delimiter}1234567890124`,
    `Product3${delimiter}abc${delimiter}1234567890125`,
  ]
...
describe('KonzumAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('SUPERMARKET,VALKANELA 10 52450 VRSAR,0613,43525,29.12.2025, 05-20.CSV')
  ...
  describe('validateRow()', () => {
    ...
    it('should warn about invalid barcode format', () => {
      ...
      const result = adapter.validateRow(row)
      expect(result.warnings.some(w => w.toLowerCase().includes('barcode'))).toBe(true)
    })
  })
  ...
  describe('edge cases', () => {
    ...
    it('should handle file with only headers', async () => {
      const headers = ['Šifra', 'Naziv', 'Kategorija', 'Marka', 'Mjerna jedinica', 'Količina', 'Cijena', 'Barkod']
...
describe('LidlAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('Supermarket 265_Ulica Franje Glada_13_40323_Prelog_1_30.11.2025_7.15h.csv')
      const result = adapter.extractStoreIdentifier(file)
  ...
  describe('validateRow()', () => {
    ...
    it('should warn about missing GTIN', () => {
      ...
      const result = adapter.validateRow(row)
      expect(result.warnings.some(w => w.toLowerCase().includes('gtin') || w.toLowerCase().includes('barcode'))).toBe(true)
    })
  })
})
...
describe('PlodineAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('SUPERMARKET_BANA_JOSIPA_JELACICA_158A_34310_PLETERNICA_163_229_29122025015054.csv')
      const result = adapter.extractStoreIdentifier(file)
...
describe('IntersparAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('hipermarket_zadar_bleiburskih_zrtava_18_8701_interspar_zadar_0242_20251229_0330.csv')
      const result = adapter.extractStoreIdentifier(file)
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from metadata if present', () => {
      const file: DiscoveredFile = {
        ...
        filename: 'SUPERMARKET-Bijela_uvala_5_FUNTANA-T598-229-2026-12-29-07-00-14-559375.xml',
        type: 'xml',
        size: null,
        lastModified: null,
...
describe('KauflandAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('Hipermarket_114__Brigade_6_Split_1630_10102025_7-30.csv')
      const result = adapter.extractStoreIdentifier(file)
...
describe('EurospinAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('diskontna_prodavaonica-310002-I_Stefanovecki_zavoj_12-Zagreb-10000-310002311025-31.10.2025-7.30.csv')
      const result = adapter.extractStoreIdentifier(file)
...
describe('DmAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should return national store identifier', () => {
      const file = createDiscoveredFile('vlada-oznacavanje-cijena-cijenik-229-data.xlsx', 'xlsx')
      const result = adapter.extractStoreIdentifier(file)
...
describe('KtcAdapter', () => {
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('TRGOVINA-IVANECKO NASELJE 1   C IVANEC-PJ58-1-20251130-071002.csv')
      const result = adapter.extractStoreIdentifier(file)
...
describe('MetroAdapter', () => {
  ...
  beforeAll(() => {
    ...
    sampleFiles = getSampleFiles('metro')
  })
...
describe('TrgocentarAdapter', () => {
  ...
  beforeAll(() => {
    ...
    sampleFiles = getSampleFiles('trgocentar')
  })
  ...
  describe('extractStoreIdentifier()', () => {
    it('should extract store identifier from filename', () => {
      const file = createDiscoveredFile('SUPERMARKET_HUM_NA_SUTLI_185_HUM_NA_SUTLI_P220_209_291220250746.csv')
      const result = adapter.extractStoreIdentifier(file)
...
Test Product;123;Brand;1 kg;KOM;12,99;12,99;;;12,99;1234567890123;FOOD;`
  ...
  describe('barcode validation', () => {
    it('should accept valid EAN-13 barcodes', () => {
      ...
      const result = adapter.validateRow(row)
      expect(result.warnings.filter(w => w.includes('barcode'))).toHaveLength(0)
    })
    ...
    it('should warn about invalid barcode formats', () => {
      ...
      const result = adapter.validateRow(row)
      expect(result.warnings.filter(w => w.toLowerCase().includes('barcode')).length).toBeGreaterThan(0)
    })
  })
  ...
  describe('discount price validation', () => {
    ...
    it('should accept valid discount price less than regular price', () => {
      ...
      const result = adapter.validateRow(row)
      expect(result.warnings.filter(w => w.toLowerCase().includes('discount'))).toHaveLength(0)
    })
  })

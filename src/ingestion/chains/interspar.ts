...
export class IntersparAdapter extends BaseCsvAdapter {
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		try {
			...
			if (!response.ok) {
				console.error(
					`Failed to fetch Interspar JSON API: ${response.status} ${response.statusText}`,
				);
			...
			console.log(
				`[DEBUG] Found ${data.files.length} file(s) in JSON response`,
			);
	...
	extractStoreMetadata(file: DiscoveredFile): StoreMetadata | null {
		...
		const intersparIdx = parts.findIndex(
			(p) => p.toLowerCase() === "interspar",
		);

...
export class LidlAdapter extends BaseCsvAdapter {
	...
	async discover(): Promise<DiscoveredFile[]> {
		...
		try {
			...
			if (!response.ok) {
				console.error(
					`Failed to fetch Lidl portal: ${response.status} ${response.statusText}`,
				);
			...
			const downloadPattern =
				/href=["'](https:\/\/tvrtka\.lidl\.hr\/content\/download\/\d+\/fileupload\/([^"']+\.zip))["']/gi;
			...
			if (this.discoveryDate && discoveredFiles.length === 0) {
				...
				const relativePattern =
					/href=["'](\/content\/download\/\d+\/fileupload\/([^"']+\.zip))["']/gi;
	...
	validateRow(row: NormalizedRow): NormalizedRowValidation {
		...
		const gtinWarnings = warnings.filter(
			(w) => !w.includes("Invalid barcode format"),
		);
		...
		for (const barcode of row.barcodes) {
			if (!/^\d{8}$|^\d{13}$|^\d{14}$/.test(barcode)) {
				gtinWarnings.push(
					`Invalid GTIN format: ${barcode} (expected EAN-8, EAN-13, or GTIN-14)`,
				);
			}
		}

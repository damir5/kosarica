...
async function handleParseChunked(
  ...
  const filename = innerFilename || file.filename
  console.log(`[parse_chunked] Parsing ${filename} with chunk size ${chunkSize}`)
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    const endRow = Math.min(startRow + chunkSize - 1, parseResult.rows.length - 1)
...
async function handleRerun(
  ...
  console.log(`[rerun] Re-running ${targetType} ${targetId} from run ${originalRunId}`)
  ...
  switch (targetType) {
    case 'run': {
      ...
      console.log(`[rerun] Re-running ${files.length} file(s) from run ${targetId}`)
  ...
  try {
    switch (taskType) {
      ...
      case 'verify_address': {
        ...
        console.log(`[enrich_store] Address verification pending for store ${storeId}`)
        break
      }

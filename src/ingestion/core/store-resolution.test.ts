...
import { resolveStoreId } from './persist'
import { CHAIN_CONFIGS, type ChainId } from '../chains/index'
...
import type { DiscoveredFile, StoreIdentifier } from './types'
...
/**
 * Creates a mock database for testing store resolution
 */
function createMockDb(
  ...
  const mockDb = {
    ...
    limit: vi.fn().mockImplementation(function (this: unknown) {
      // Access closure variables through the mock implementation
  ...
  // Create a more sophisticated mock that actually filters data
  const createSelectMock = () => {
    let chainSlugFilter: string | null = null
    let identifierTypeFilter: string | null = null
    let identifierValueFilter: string | null = null
    ...
    const chain = {
      ...
      where: vi.fn().mockImplementation((condition) => {
        // Parse the condition to extract filter values
        // This is a simplified mock that expects specific filter patterns
        return chain
      }),
      limit: vi.fn().mockImplementation((n: number) => {
        ...
        const matchingIdentifier = identifiers.find((id) => {
          ...
          if (chainSlugFilter && store.chainSlug !== chainSlugFilter)
            return false
          if (identifierTypeFilter && id.type !== identifierTypeFilter)
            return false
          if (identifierValueFilter && id.value !== identifierValueFilter)
            return false
        ...
        if (matchingIdentifier) {
          return Promise.resolve([{ storeId: matchingIdentifier.storeId }])
        }
        return Promise.resolve([])
      }),
    }
  ...
  // Override the mock to use filter-aware implementation
  return {
    select: vi.fn().mockImplementation(({ storeId }) => {
      return {
        from: vi.fn().mockReturnValue({
          innerJoin: vi.fn().mockReturnValue({
            where: vi.fn().mockImplementation((condition) => {
              return {
                limit: vi.fn().mockImplementation((n: number) => {
                  // This mock needs access to the actual filter values
                  // For testing purposes, we'll use a callback pattern
                  return Promise.resolve([])
                }),
              }
            }),
...
function createTestDb(
  ...
  // Create a mock db that uses the resolve function
  const db = {
...
describe('Filename-First Resolution', () => {
  ...
  describe('Store resolution lookup', () => {
    it('finds store by filename_code identifier', () => {
      ...
      const storeId = resolveStore('konzum', 'Zagreb_Ilica_123', 'filename_code')
    ...
    it('respects chain slug when resolving', () => {
      ...
      const wrongChain = resolveStore('lidl', 'Zagreb_Ilica_123', 'filename_code')
    ...
    it('matches exact identifier value (case sensitive)', () => {
      ...
      const lowercase = resolveStore('konzum', 'zagreb_ilica_123', 'filename_code')
      const uppercase = resolveStore('konzum', 'ZAGREB_ILICA_123', 'filename_code')
      const correct = resolveStore('konzum', 'Zagreb_Ilica_123', 'filename_code')

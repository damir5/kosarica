...
function getStatementParamCount(statement: unknown, context: string): number {
	if (!statement || typeof statement !== "object") {
		throw new Error(
			`${context}: expected a Drizzle statement, received ${typeof statement}`,
		);
	}
	...
	if (typeof query.toSQL !== "function") {
		throw new Error(
			`${context}: statement is missing a toSQL() method, cannot determine parameter count`,
		);
	}
	...
	if (!result || !Array.isArray(result.params)) {
		throw new Error(
			`${context}: statement toSQL() did not return a params array`,
		);
	}
...
>(
	...
	for (const row of data) {
		...
		const singleRowParams = getStatementParamCount(
			singleRowQuery,
			"batchInsert",
		);
...
export async function batchExecute<TItem>(
	...
	for (const item of items) {
		...
		if (
			pendingStatements.length > 0 &&
			currentParams + params > D1_MAX_PARAMS
		) {
			await flush();
		}

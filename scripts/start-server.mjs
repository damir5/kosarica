...
const WORKERS = Number.parseInt(process.env.WEB_CONCURRENCY ?? "2", 10);

// ── Primary process: fork workers ────────────────────────────────────

if (cluster.isPrimary && WORKERS > 1) {
	console.log(
		`Primary ${process.pid}: forking ${WORKERS} workers (WEB_CONCURRENCY=${WORKERS})`,
	);

	for (let i = 0; i < WORKERS; i++) {
		const w = cluster.fork({ WORKER_INDEX: String(i) });
		console.log(`  Started worker ${w.process.pid} (index ${i})`);
	}
	...
	// ── Worker process (or single-process mode when WEB_CONCURRENCY=1) ───

	const workerIndex = Number.parseInt(process.env.WORKER_INDEX ?? "0", 10);

	// Only first worker runs background services (scheduler + task queue).
	// Additional workers are HTTP-only.
	if (workerIndex > 0) {
		process.env.SKIP_BACKGROUND_SERVICES = "1";
	}
	...
	function getRequestUrl(req) {
		...
		return `${protocol}://${hostHeader}${reqPath}`;
	}

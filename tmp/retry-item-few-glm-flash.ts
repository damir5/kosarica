...
function decideFinalStatus(state: string): 'APPROVED' | 'REJECTED' | 'PENDING_REVIEW' | 'SYSTEM_ERROR' {
  if (state === 'AUTO_APPROVED') return 'APPROVED';
  if (state === 'AUTO_REJECTED') return 'REJECTED';
  if (state === 'SYSTEM_ERROR') return 'SYSTEM_ERROR';
...
async function main() {
  ...
  const unresolvedResult = await db.execute(sql`
    ...
    WHERE (d.item_a_id = ${ITEM_ID} OR d.item_b_id = ${ITEM_ID})
      AND d.final_status IN ('SYSTEM_ERROR', 'PENDING_REVIEW')
    ORDER BY d.updated_at ASC
    LIMIT ${LIMIT}
  `);

  const unresolved = (Array.isArray(unresolvedResult)
    ? unresolvedResult
    : unresolvedResult.rows ?? []) as PairRow[];
  ...
  for (const row of unresolved) {
    ...
    else if (finalStatus === 'REJECTED') rejected += 1;
    else if (finalStatus === 'PENDING_REVIEW') pendingReview += 1;
    else systemErrors += 1;

...
type ClusterStats = {
  ...
  itemCount: number;
  quoteCount: number;
  priceEurMedian: number | null;
  priceEurP10: number | null;
  priceEurP90: number | null;
  eurPerLMedian: number | null;
  eurPerLP10: number | null;
  eurPerLP90: number | null;
  examples: string[];
};
...
async function main() {
  ...
  const colaItems = toRows<ColaItemRow>(colaItemsResult).filter((row) =>
    row.total_amount != null && Number.isFinite(row.total_amount) && row.total_amount > 0,
  );

  const itemIds = colaItems.map((row) => row.id);
  if (itemIds.length === 0) {
    console.log(JSON.stringify({ message: 'No cola beverage items with parsed liters found.' }, null, 2));
    return;
  }
  ...
  const itemMap = new Map(colaItems.map((row) => [row.id, row]));
  ...
  const clusters: ClusterStats[] = Array.from(clusterByKey.entries()).map(([key, entry]) => {
  ...
  clusters.sort((a, b) => b.quoteCount - a.quoteCount || b.itemCount - a.itemCount);

  const parseAnomalies = colaItems.filter((row) =>
    (row.pack_amount ?? 1) === 1 && (row.total_amount ?? 0) >= 3,
  );
  ...
  const report = {
    scope: {
      filter: "cola beverages: extracted_unit='l' and name/brand contains 'cola'",
      colaItems: itemIds.length,
      pricedQuotes: latestPrices.length,
      latestPriceDate: latestDateRows[0]?.latest_date ?? null,
    },
    topClusters: clusters.slice(0, 20),
    parseQuality: {
      packAmountOneWithTotalAtLeast3L: parseAnomalies.length,
      sample: parseAnomalies.slice(0, 12).map((row) => ({
        id: row.id,
        name: row.name,

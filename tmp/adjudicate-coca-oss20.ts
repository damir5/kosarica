...
async function main() {
  ...
  const unresolved = getRows<PairRow>(unresolvedRows);
  const batchSize = 20;
  const concurrency = 12;
  const batches = chunkArray(unresolved, batchSize);
  ...
  async function processBatch(batch: PairRow[]) {
    ...
    if (processedBatches % 25 === 0) {
      console.log(`progress: ${processedBatches}/${batches.length} batches`);
    }
  }

  for (let index = 0; index < batches.length; index += concurrency) {
    const window = batches.slice(index, index + concurrency);
    await Promise.all(window.map((batch) => processBatch(batch)));
  }
  ...
  const finalStatusRows = await db.execute(sql`
    ...
    JSON.stringify(
      {
        ...
        unresolvedAtStart: unresolved.length,
        adjudicationOutcome: { approved, rejected, pendingReview, systemErrors },
        rebuildRun: {
          variantClusters: rebuildRun.variantClusters,
          baseClusters: rebuildRun.baseClusters,
        },
        finalStatusCounts: getRows<{ final_status: string; c: number }>(finalStatusRows),
      },
      null,
      2,
    ),

...
async function main(): Promise<void> {
  ...
  const rowsResult = await db.execute(sql`
    ...
    banana_seeds AS (
      ...
      JOIN retailer_items ri ON ri.id = f.retailer_item_id
      WHERE (
        lower(coalesce(f.normalized_name, '')) LIKE '%banana%'
        OR lower(coalesce(ri.name, '')) LIKE '%banana%'
        OR lower(coalesce(f.normalized_name, '')) LIKE '%banane%'
        OR lower(coalesce(ri.name, '')) LIKE '%banane%'
      )
      AND lower(coalesce(f.normalized_name, '')) NOT LIKE '%bananko%'
      AND lower(coalesce(ri.name, '')) NOT LIKE '%bananko%'
    ),
    milk_seeds AS (
      SELECT
        'milk'::text AS topic,
      ...
      JOIN retailer_items ri ON ri.id = f.retailer_item_id
      WHERE (
        lower(coalesce(f.normalized_name, '')) LIKE '%mlijek%'
    ...
    all_seeds AS (
      SELECT * FROM cola_seeds
      UNION ALL
      SELECT * FROM banana_seeds
      UNION ALL
      SELECT * FROM milk_seeds
    ),
    ranked_seeds AS (
      SELECT
        *,
    ...
    candidate_pool AS (
      ...
      WHERE (
        (s.embedding IS NOT NULL AND t.embedding IS NOT NULL AND (1 - (s.embedding <=> t.embedding)) >= 0.60)
        OR similarity(s.normalized_name, t.normalized_name) >= 0.28
      )
    ),
    dedup AS (
    ...
    ranked_topic AS (
      SELECT
        *,
        row_number() OVER (PARTITION BY topic ORDER BY emb_sim DESC NULLS LAST, lex_sim DESC NULLS LAST) AS topic_rank
      FROM dedup
    )
    SELECT *
    FROM ranked_topic
    WHERE topic_rank <= 60
    ORDER BY topic, topic_rank
  `);

  const rows = getRows<Row>(rowsResult);
  console.log(`Selected pairs: ${rows.length}`);
  if (rows.length === 0) {
    console.log('No pairs found for requested topics.');
    return;
  }

  const pairResults: PairOut[] = [];
  const batchSize = Number.parseInt(process.env.SEMANTIC_CLUSTERING_LLM_PROMPT_BATCH_SIZE ?? '25', 10) || 25;
  ...
  for (let offset = 0; offset < rows.length; offset += batchSize) {
    ...
    for (const row of batchRows) {
      ...
      pairResults.push({
        ...
        voteLatencyMs: result?.votes[0]?.latencyMs ?? null,
  ...
  const byTopic = (['cola', 'banana', 'milk'] as const).map((topic) => {
    const topicRows = pairResults.filter((p) => p.topic === topic);
    return {
      topic,
      pairs: topicRows.length,
      avgConfidence: avg(topicRows.map((p) => p.confidence)),
      avgVoteLatencyMs: avg(topicRows.map((p) => p.voteLatencyMs).filter((v): v is number => v !== null)),
      verdicts: countBy(topicRows.map((p) => p.verdict)),
      states: countBy(topicRows.map((p) => p.decisionState)),
      sample: topicRows.slice(0, 10),
    };
  });
  ...
  const report = {
    ...
    wallClockPairsPerMinute: pairResults.length > 0 ? (pairResults.length * 60000) / totalMs : 0,
    avgVoteLatencyMs: avg(latencies),
    verdictsOverall: countBy(pairResults.map((p) => p.verdict)),
    statesOverall: countBy(pairResults.map((p) => p.decisionState)),
    byTopic,
  };

  const outPath = `/tmp/keyword-match-eval-v2-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
  writeFileSync(outPath, JSON.stringify(report, null, 2));

  console.log('\n=== Keyword Match Eval v2 ===');
  console.log(`Output: ${outPath}`);
  console.log(`Model config: ${report.envModelConfig}`);
  console.log(`Total pairs: ${report.totalPairs}`);
  console.log(`Duration: ${report.totalDurationMs} ms`);
  console.log(`Wall throughput: ${report.wallClockPairsPerMinute.toFixed(2)} pairs/min`);
  ...
  console.log(`Overall states: ${JSON.stringify(report.statesOverall)}`);
  for (const topic of report.byTopic) {
    console.log(`Topic ${topic.topic}: pairs=${topic.pairs}, avgLatency=${topic.avgVoteLatencyMs.toFixed(2)}ms, verdicts=${JSON.stringify(topic.verdicts)}, states=${JSON.stringify(topic.states)}`);
  }
}
